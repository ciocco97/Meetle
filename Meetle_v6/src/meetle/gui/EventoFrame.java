...
public class EventoFrame extends javax.swing.JFrame {
    ...
    public EventoFrame(Meetle meetle, int eventoID, int mode) {
        this.meetle = meetle;
        initComponents();
        this.eventoID = eventoID;
        this.mode = mode;
        for(Campo campoT: this.meetle.getBacheca().getByID(eventoID).getTuttiCampi()) // qualche altro metodo?
            jPanelCampi.add(new CampoPanel(campoT, mode!=VISUALIZZA));
        String titolo = this.meetle.getBacheca().getByID(eventoID).getTitolo();
        if (titolo == null)
        ...
        switch(mode){
            ...
            case MODIFICA:jButton2.setText("Elimina");
                jButton2.addActionListener((ActionEvent e) -> this.meetle.getBacheca().rimuoviByID(eventoID));
...
//        }
    ...
    public boolean salvaEventoEChiudi() {
        ...
        this.meetle.getBacheca().getByID(eventoID).aggiornaStato();
